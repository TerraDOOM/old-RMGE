...
use rmge::{WindowState, HalState, geometry::Quad, graphics::{TexturedQuad, HalStateBuilder}};
...
fn main() {
    ...
    let mut hal_state = match HalStateBuilder::new(&winit_state, JUDGEMENT_BYTES)
        .num_quads(1)
        .finish() {
            Ok(state) => state,
            Err(e) => panic!(e),
    ...
    loop {
        ...
        if let Some(a) = inputs.new_frame_size {
            ...
            hal_state = match HalStateBuilder::new(&winit_state, JUDGEMENT_BYTES)
                .num_quads(1)
                .finish() {
                    Ok(state) => state,
                    Err(e) => panic!(e),
