...
use winit::{EventsLoop, Window, WindowEvent};
...
fn main() {
    ...
    let mut hal_state = match HalState::new(&window, "rustmania", 512, [Mailbox, Fifo, Relaxed, Immediate], log) {
        Ok(state) => state,
        Err(e) => panic!(e),
    };
    let (frame_width, frame_height) = window
      ...
      let mut local_state = LocalState {
        ...
        hal_state.load_texture(include_bytes!("creature-smol.png"));
        ...
        loop {
            ...
            if let Some(a) = inputs.new_frame_size {
                ...
                hal_state = match HalState::new(&window, "rustmania", 512, [Mailbox, Fifo, Relaxed, Immediate], log) {
                    Ok(state) => state,
                    Err(e) => panic!(e),
                };
            }
